...
- Public Dropbox `/scl/fo/` folder submissions default to direct archive download. Each start request may explicitly choose backend expansion and independently choose whether the persisted suffix list applies. Expansion stays entirely inside TrueDown without Chrome or a Developer API token: the isolated adapter establishes an anonymous CSRF session, derives the current `secure_hash`, calls `list_shared_link_folder_entries`, follows signed vouchers, traverses sibling folders with bounded parallelism, then bulk-adds all accepted stable `dl=1` tasks while preserving hierarchy and duplicate/resume behavior. KDownloader can synchronize the suffix list and default choice through the authenticated configuration path. Neither frontend calls or parses Dropbox. The private web protocol is intentionally isolated because Dropbox may change it.
