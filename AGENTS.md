...
- The HTTP boundary validates request hosts and write origins, applies security headers and timeouts, limits start requests to 1 MiB, accepts exactly one known-field JSON object, and returns task snapshots without headers or aria2 options. `GET/POST /settings/download-rules` persists the bounded Dropbox mode and filter defaults; filter-only posts from older KDownloader clients preserve the current mode. `GET/POST /settings/runtime` persists aria2-wide settings including concurrent downloads, and `GET/POST /modules` lists or changes resolver enablement. `POST/DELETE /modules/package` imports a package of at most 64 KiB or restores its embedded baseline. Start requests carry bounded, module-owned options under `moduleOptions`. KDownloader never repeats filter rules inside ordinary task requests. KDownloader, AB's extension, and the TrueDown dashboard all use the same `X-Api-Key` authentication header.
...
- `truedown/build.ps1` creates the TrueDown Windows package with `TrueDown.exe`, `aria2c.exe`, and the third-party notices. TrueDown tags use the `truedown-build-*` prefix so they cannot collide with KDownloader tags.
