...
func TestBundledStableAria2MatchesReviewedOfficialBuild(t *testing.T) {
	data, err := os.ReadFile(repositoryFile(t, "aria2", "aria2c.exe"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildScriptGuardsRecursiveDeleteAndCopiesAgainstReparsePoints(t *testing.T) {
	data, err := os.ReadFile(repositoryFile(t, "build.ps1"))
	if err != nil {
		t.Fatal(err)
	}
	script := string(data)
...
func TestWindowsIconContainsShellSizes(t *testing.T) {
	data, err := os.ReadFile(repositoryFile(t, "windows", "truedown.ico"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestMacIconContainsModernPNGRepresentations(t *testing.T) {
	data, err := os.ReadFile(repositoryFile(t, "macos", "truedown.icns"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnixBuildScriptPackagesLinuxAndMacOS(t *testing.T) {
	data, err := os.ReadFile(repositoryFile(t, "build-unix.sh"))
	if err != nil {
		t.Fatal(err)
	}
	script := string(data)
...
func TestWSL2PreparationBuildsApplicationAndTestBinaries(t *testing.T) {
	data, err := os.ReadFile(repositoryFile(t, "tools", "prepare-wsl-tests.ps1"))
	if err != nil {
		t.Fatal(err)
	}
	script := string(data)
