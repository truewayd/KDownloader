...
try {
  ...
  await waitForNativeCondition(settings, () => document.activeElement?.closest('[data-group-id^="group-"]'));
  assert.equal(await settings.locator('[data-group-id^="group-"]').count(), 1);
  await invoke(main, "open_group_settings", { groupId: "image", add: false });
  await waitForNativeCondition(settings, () => document.activeElement?.closest("[data-group-id]")?.dataset.groupId === "image");
  assert.equal(await settings.locator('[data-group-id^="group-"]').count(), 1, "group navigation preserves unsaved drafts");
  await settings.locator('[data-group-id^="group-"] [data-remove-group]').click();
  await waitForNativeCondition(settings, () => fileGroupsDraft.length === 8 && !fileGroupsSaving && !fileGroupsSaveQueued);
