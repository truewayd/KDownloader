...
- 默认监听 `127.0.0.1:15151`，仅提供 CLI 和扩展使用的 API；图形界面由 Tauri 打包。
- 设置采用左侧分类、右侧设置项，默认进入「下载与速度」，日志和关于位于设置内；重新打开保留未保存的编辑。
- 默认提供图片、视频、音乐、压缩包、应用、文档、工程、其他分组，支持自定义分组、后缀和图标；任务状态通过下拉框筛选。
- 支持单个任务的信息与设置页；下载页默认只显示常用字段。代理默认使用系统设置，可选择自定义 HTTP(S) 代理或无代理。
- 日志自动刷新并跟随最新内容，按天或 4 MiB 轮转，最多保留 3 份归档、最长 7 天。
- Windows 正式桌面包支持空闲时自动更新及失败回滚，Aria2 Next 有独立的自动更新开关。
- 按操作系统提供登录时启动、托盘与窗口风格，Windows 支持 Mica 和按显示器 DPI 选择托盘图标，macOS 支持系统材质与 Retina 模板图标。
- 桌面任务界面支持新建、筛选、分页、表头正逆序排序、暂停、继续、重试、打开和移除任务，并提供整条队列的暂停与恢复。
...
设置保留原有分类顺序，移除总览，每页独立保存。在「设置 → 启动与运行」可开启登录时自动启动，登录后驻留托盘。
